...
import type { VerificationAction, TimelineEntry as SharedTimelineEntry } from "@/types/contracts";
...
function actorLabel(entry: SharedTimelineEntry): string {
...
function TimelineEntryUI({ entry, isLast }: { entry: SharedTimelineEntry; isLast: boolean }) {
...
export function Timeline({ entries }: { entries: SharedTimelineEntry[] }) {
  if (entries.length === 0) {
  ...
  return (
    <ol className="min-w-0" aria-label="Application progress">
      {ordered.map((entry, index) => (
        <TimelineEntryUI key={entry.id} entry={entry} isLast={index === ordered.length - 1} />
      ))}
    </ol>
  );
}
