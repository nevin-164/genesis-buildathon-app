...
function formatSubmitted(application: InternshipListItem): string {
...
function statusAction(application: InternshipListItem): StatusAction {
...
export function ApplicationCard({ application }: { application: InternshipListItem }) {
  ...
  return (
    <article
      className={cn(
        ...
        application.status === "verified" && "border-[#b8d4bc] bg-[#f4f8f5]",
        application.status === "changes_requested" && "border-amber-200/80",
      )}
    >
      <span
      ...
      <div className="flex flex-1 flex-col p-4 sm:p-5">
        <header className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge tone={INTERNSHIP_STATUS_TONE[application.status]}>
              {INTERNSHIP_STATUS_LABEL[application.status]}
            </Badge>
          </div>
