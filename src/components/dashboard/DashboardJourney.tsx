...
export function DashboardJourney({ dashboard }: { dashboard: StudentDashboard }) {
  ...
  const needsChanges = dashboard.nextAction === "fix_internship";
  ...
  return (
    <section
      className={cn(
        ...
        MOTION,
        step.urgent && !needsChanges && "border-amber-200/80 bg-amber-50/25",
        needsChanges && "border-amber-200/70 bg-[#fffbf5]",
      )}
