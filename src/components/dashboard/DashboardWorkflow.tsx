...
function InternshipWorkflowCard({ internship }: { internship: InternshipListItem }) {
  const submittedText = internship.submittedAt
    ? formatDisplayDate(internship.submittedAt)
  ...
  const href =
    internship.status === "draft" || internship.status === "changes_requested"
      ? `/student/internship/${internship.id}/edit`
      : `/student/internship/${internship.id}`;
  const cta =
    internship.status === "draft" || internship.status === "changes_requested"
      ? "Continue internship"
      : "View internship";
...
export function DashboardWorkflow({ dashboard }: { dashboard: StudentDashboard }) {
  const hasInternship = dashboard.internship !== null;
  ...
  return (
    <section className="min-w-0 space-y-4" aria-labelledby="dashboard-workflow-heading">
      ...
      <div className="grid min-w-0 gap-4">
        {hasInternship && dashboard.internship && (
