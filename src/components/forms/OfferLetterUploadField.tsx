...
export function OfferLetterUploadField({
  ...
  return (
    <ApplicationField
      ...
      required
      error={error}
      hint={`Accepted formats: ${OFFER_LETTER_ACCEPT_LABEL}. ${OFFER_LETTER_MAX_SIZE_LABEL}.`}
      className="sm:col-span-2"
    >
      <input type="hidden" name="offerLetterEvidenceId" value={evidence?.id ?? ""} />

      {evidence && phase === "success" && (
        <div className={cn(PANEL, "mb-3 flex min-w-0 flex-col gap-2 bg-[#f4f8f5] p-3 sm:flex-row sm:items-center sm:justify-between")}>
