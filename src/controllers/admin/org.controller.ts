...
/**
 * The advisor dropdown on the class screens reads this. Same list as
 * `user.controller.listFacultyOptions` — re-exported here so an org screen
 * imports only from the org controller.
 *
 * Passing null to `updateClassAdvisor` removes the advisor.
 */
...
export async function updateClassAdvisor(
  _classId: string,
  _advisorId: string | null,
): Promise<void> {
  await requireRole("admin");
}

/**
 * Detaches a student from a class. Nothing is deleted — the student row stays,
 * it just stops resolving an advisor through this class.
 */
