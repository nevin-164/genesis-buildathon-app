...
export default async function MyInternshipsPage() {
  ...
  const internships = await listMyInternships();
  const singleInternship = internships.length === 1;
      ...
      <div className={cn(EXPLORE_PAGE, "mx-auto w-full max-w-5xl min-w-0")}>
          ...
          <div className="flex flex-col gap-4 pl-3 sm:flex-row sm:items-end sm:justify-between">
            <div className="min-w-0">
              ...
              <p className={cn("mt-1 max-w-xl text-[15px] leading-snug", MUTED)}>
                Track verification and see what needs your attention.
              </p>
            </div>
            <Link
        ...
        {internships.length === 0 ? (
          ...
          <ul
            className={cn(
              "grid gap-4",
              singleInternship ? "grid-cols-1" : "md:grid-cols-2",
            )}
            aria-label="Your internships"
          >
            {internships.map((internship) => (
              <li
                key={internship.id}
                className={cn("flex min-w-0", singleInternship && "col-span-full")}
              >
