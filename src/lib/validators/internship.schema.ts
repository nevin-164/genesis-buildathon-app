...
const workNatureValues = WORK_NATURES.map((w) => w.value) as [string, ...string[]];
const mentorFreqValues = MENTOR_FREQUENCIES.map((m) => m.value) as [string, ...string[]];
const appSourceValues = APPLICATION_SOURCES.map((s) => s.value) as [string, ...string[]];
